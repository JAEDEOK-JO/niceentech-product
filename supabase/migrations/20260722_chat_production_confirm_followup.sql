...
BEGIN
  ...
  IF v_trigger.file_type = 'production_confirm' THEN
    IF coalesce(v_trigger_content->>'status', '') <> 'in_progress' THEN
  ...
  IF v_content ? 'confirmation'
    ...
    -- 최초 요청 카드에서 이미 완료된 경우만 차단.
    -- 진행중이면 확인카드(production_confirm) 경로로만 후속 허용.
