...
const { loading, errorMessage, monthlyMetrics, currentMonthMetrics, totalBalanceTon, fetchTotalReportData } = useTotalReportData()
...
const periodLabel = computed(() => {
  ...
  return `${list[0].label} ~ ${list[list.length - 1].label} (${list.length}개월)`
})
...
const analysisGroups = computed(() => buildDetailedAnalysis(monthlyMetrics.value, valueChain.value))
    ...
    bars: [
      {
        ...
        display: Number(month.drawingHeadQty).toLocaleString('ko-KR'),
        color: '#10b981',
      },
      {
        label: '생산완료',
        value: month.productionHeadQty,
        display: Number(month.productionHeadQty).toLocaleString('ko-KR'),
        color: '#f43f5e',
      },
    ],
  })),
  ...
  return [
    {
      ...
      subLabel: '헤드당 평균 매출',
      subValue: `${Math.round(simulation.value.salesPerHead).toLocaleString('ko-KR')}원`,
    },
    {
      key: 'design',
      department: '설계부',
      ...
      <div v-else class="space-y-6">
        ...
        <section class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm md:p-6">
          ...
          <div class="mt-4 grid gap-4 xl:grid-cols-3">
            ...
            />
            <TotalReportBarChart title="도면배포 · 생산 (헤드)" :groups="workChartGroups" />
        ...
        <section class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm md:p-6">
          <div class="flex items-center justify-between gap-3">
            ...
            <p v-if="simulation" class="text-[12px] text-slate-500">{{ simulation.baseMonthLabels }} 실적 기준</p>
          </div>
