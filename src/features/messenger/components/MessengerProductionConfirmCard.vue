...
import type { ConfirmOptionId, ProductionConfirmPayload } from '../utils/productionRequestMessage'
...
const props = defineProps<{
  payload: ProductionConfirmPayload
  messageId: string
  senderName?: string
  confirming?: boolean
}>()
...
const canReconfirm = computed(
  () => props.payload.status === 'in_progress' && !props.payload.closed,
)

const showConfirmButton = computed(() => props.payload.status === 'in_progress')
...
<template>
  <article
    class="w-[400px] max-w-[calc(100vw-96px)] overflow-hidden rounded-2xl border border-emerald-200 bg-white shadow-sm"
  >
    <div class="border-b border-emerald-100 bg-emerald-50 px-3.5 py-2.5">
      <div class="flex items-center justify-between gap-2">
        <p class="text-xs font-extrabold tracking-wide text-emerald-700">작업확인 완료</p>
        <span
      ...
      <p class="mt-0.5 text-xs font-extrabold text-emerald-800">{{ payload.optionLabel }}</p>
      ...
      <p v-if="payload.requestTypeLabel" class="mt-1.5 text-[11px] font-bold text-slate-400">
    ...
    <div class="space-y-3 px-3.5 py-3">
      ...
      <div v-if="payload.recipients?.length">
        ...
        <div class="mt-1.5 flex flex-wrap gap-1.5">
          <span
            ...
            :key="name"
            class="rounded-lg bg-emerald-50 px-2 py-1 text-[11px] font-extrabold text-emerald-700"
          >
            {{ name }}
          </span>
        </div>
      ...
      <div class="rounded-xl bg-slate-50 px-3 py-2">
        ...
        <p class="mt-1 text-sm font-semibold text-slate-800">
          {{ payload.confirmedByName }} · {{ formatConfirmDateTime(payload.confirmedAt) }}
        </p>
        <p class="mt-0.5 text-xs font-extrabold text-emerald-700">{{ payload.optionLabel }}</p>
        <p v-if="payload.recheckAt && canReconfirm" class="mt-0.5 text-xs text-slate-500">
          재확인 예정 · {{ formatConfirmDateTime(payload.recheckAt) }}
        </p>
      </div>
      ...
      <div class="flex gap-2">
        ...
        <button
          ...
          :class="
            !canReconfirm || confirming
              ? 'cursor-not-allowed border border-slate-200 bg-slate-100 text-slate-400'
              : 'border border-indigo-200 bg-indigo-50 text-indigo-700 hover:bg-indigo-100'
          "
          :disabled="!canReconfirm || confirming"
