...
const assignedDepartments = computed(() => {
  const set = new Set(props.employees.map((e) => e.assignedDepartment).filter(Boolean))
  return ASSIGNED_DEPT_ORDER.filter((d) => set.has(d))
})
...
const visibleEmployees = computed(() => {
  ...
  const list = selectedDept.value === '__ALL__'
    ? props.employees
    : props.employees.filter((e) => e.assignedDepartment === selectedDept.value)
...
watch(selectedDept, () => {
  ...
  for (const emp of visibleEmployees.value) {
    ...
    if (existing) next.set(emp.id, existing.endTime)
  }
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
      <div class="w-full max-w-2xl max-h-[90vh] flex flex-col rounded-2xl bg-white shadow-2xl">
        ...
        <div class="flex items-start justify-between border-b border-slate-200 p-5">
          <div>
            <h2 class="text-lg font-extrabold text-slate-900">오늘 근무시간 입력</h2>
          ...
          <div v-else class="space-y-2">
            ...
            >
              <div class="flex min-w-0 items-center gap-2">
                ...
                <span class="text-[11px] font-bold text-slate-400">{{ emp.assignedDepartment || emp.department || '-' }}</span>
              </div>
        ...
        <div class="flex items-center justify-between gap-3 border-t border-slate-200 bg-slate-50 px-5 py-3">
          <div class="text-xs font-bold text-slate-500">
            <template v-if="selectedDept">선택됨 {{ selectedCount }} / {{ visibleEmployees.length }}명</template>
          </div>
